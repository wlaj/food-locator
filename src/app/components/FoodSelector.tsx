...
const AMSTERDAM_REGIONS = {
  ...
  "Amsterdam West": { lat: 52.372, lng: 4.872 },
};
...
export default function FoodSelector({ onSelectionChange }: FoodSelectorProps) {
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setIsLocating(false);
        if (locationIconRef.current) {
          animate(locationIconRef.current, { rotate: 0 }, { duration: 0.3 });
        }
  ...
  useEffect(() => {
    ...
    if (selectRef.current) {
      ...
      selectElements.current =
        selectRef.current.querySelectorAll(".animate-select");
  ...
  const handleDietaryChange = (value: string) => {
    const selected =
      value === "none" ? undefined : (value as DietaryPreference);
    setDietary(selected);
  ...
  return (
    <Card className="w-full shadow-sm overflow-hidden hover:shadow-md transition-all duration-300">
      <CardContent className="pt-3 pb-3">
        <div
          ref={selectRef}
          className="flex flex-wrap items-center justify-center text-xl font-medium gap-2"
        >
          <Select onValueChange={handlePersonaChange}>
            <SelectTrigger
              className={`w-auto text-xl border-none px-1 focus:ring-0 persona-trigger animate-select hover-pop ${
                persona
                  ? getColorClass("persona", persona)
          ...
          <Select onValueChange={handleCuisineChange}>
            <SelectTrigger
              className={`w-auto text-xl border-none px-1 focus:ring-0 cuisine-trigger animate-select hover-pop ${
                cuisine
                  ? getColorClass("cuisine", cuisine)
                  : "text-muted-foreground"
              }`}
            ...
            <SelectContent className="origin-top select-content" sideOffset={5}>
              {["turkish", "indonesian", "korean", "japanese"].map(
                (option, index) => (
                  <div
                    key={option}
                    className="dropdown-item select-highlight"
          ...
          <Select onValueChange={handleDietaryChange}>
            <SelectTrigger
              className={`w-auto text-xl border-none px-1 focus:ring-0 dietary-trigger animate-select hover-pop ${
                dietary
                  ? getColorClass("dietary", dietary)
                  : "text-muted-foreground"
              }`}
          ...
          <div className="flex items-center gap-1">
            <span className="text-muted-foreground">food in</span>
            <button
              ref={locationIconRef}
            ...
            >
              <LocateIcon className="w-4 h-4 text-background" />
            </button>
          </div>

          <Select onValueChange={handleLocationChange} value={location}>
            <SelectTrigger
              className={`w-auto text-xl border-none px-1 focus:ring-0 location-trigger animate-select hover-pop ${
                location
                  ? getColorClass("location", location)
                  : "text-muted-foreground"
              }`}
